...
public class UsersRepositoryImpl implements UsersRepository {
    ...
    public void save(User model) {//переопределение метода
        try {//обработка на непроверяемые ошибки
            ...
            writer.write(model.getId() + " " + model.getFirstName() + " " + model.getLastName() + " " + model.getLogin()
                    + " " + model.getPassword());//вызов обьекта записи,метод который записывает в файл необходимые данные
        ...
        try {
            ...
            while (currentline != null) {//пока есть строка
                ...
                if (currentId == id) {//если IDшник равен нужному то
                    User user = new User(userData[1], userData[2], userData[3], userData[4]);//создаем нового пользователя
